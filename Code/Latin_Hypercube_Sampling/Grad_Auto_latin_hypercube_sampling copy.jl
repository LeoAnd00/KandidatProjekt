...
function Check_if_p_works(p_var, p_const, scaled_plan)
    ...
    for item in 1:length(scaled_plan[:,1])
        ...
        # To make sure the cost isn't way to big
        #if MK > 50
        #    continue
        #end
...
"""
    ...
    First one must define a list called p_0_values_list.csv with prametervectors that the sampling will start around. This list can
    for example be created by first generating a more general latin hypercube with latin_hypercube_sampling.jl and one can then 
    filter the results to get a list with parametervectors that gave a cost below a choosen value. Then the code (main())
    will create a latin hypercube based on the Number_of_samples and what intervals the user defines. If it finds a parametervector 
    with a cost below the previous best one it will start generating a new latin hypercube around the new value. If none of the costs 
    were below the previously best one it will try to increase the number of samples by a choosen amount and generate a new latin
    hypercube. If the cost is less then the previous best but not good enough, specified by how much the user wants the cost to
    decrease, it will move on to the next parametervector in p_0_values_list.csv. The code will also save all intermediate results
    in csv files.

    Note that one must do some modifications in Check_if_p_works(p_var, p_const, scaled_plan) if the user wants to give the intervals 
    ...
    while true
        ...
        while true
            ...
            if N > 1
                ...
                #plan_scale = [(-3.0,3.0)]
                for i in 1:80
                    push!(plan_scale,(0.8*last.(p_var)[i+1],1.2*last.(p_var)[i+1]))
            ...
            if N > 1
                ...
                if Best_i == 0
                    Number_of_samples = Number_of_samples + 200 # Adds this amount to number of samples if no cost under the previous best was found
                    continue
                end
                try
                    df = CSV.read("Intermediate/p_0_values.csv", DataFrame)
            ...
            if N == 1
                ...
                N_2 += 1
                df = CSV.read("Intermediate/p_0_values_from_auto_$N_3.csv", DataFrame)
                df[!,Symbol("p_0_value_$N_2")] = vcat(last.(p_0_var),MK_current_best)
                CSV.write("Intermediate/p_0_values_from_auto_$N_3.csv",df)
            end

            if N <= 1
                MK_prev = 1e10 # Just to make sure grad_of_cost isn't less then 0.05 for the first iteration
            end

            grad_of_cost = abs(MK_df - MK_prev)
            if grad_of_cost < 0.05 # The value of which the cost must decrease from each iteration
                break
            end
