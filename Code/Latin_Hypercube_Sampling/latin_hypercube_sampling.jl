...
function Check_if_p_works(p_var, p_const, scaled_plan)
    ...
    for item in 1:length(scaled_plan[:,1])
        ...
        if N == 1
            df = DataFrame(Parameter = first.(p_var_new),
                        p_0_value_1 = last.(p_var_new)) 
            CSV.write("Intermediate/p_0_values.csv",df)
        else
            df[!,Symbol("p_0_value_$N")] = last.(p_var_new)
            CSV.write("Intermediate/p_0_values.csv",df)
        end
...
function main()
    ...
    plan, _ = LHCoptim(100,81,10) #samples, dimensions, generations to run the optimisation
    plan_scale = [(-3.0,3.0)]
    for i in 1:80
