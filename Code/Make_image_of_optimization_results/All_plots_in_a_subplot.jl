...
elseif leg_test == true
    p3 = plot([0 0 0], showaxis = false, grid = false, legend = false)
    p2 = plot([0 0 0], showaxis = false, grid = false, legend = false)
    p1 = plot([0 0 0], color = [Color_alt_1 Color_alt_2 Color_jalihal], showaxis = false, grid = false, labels = [label_alt_1 label_alt_2 label_jalihal],labelsize=100, legendfontsize=50, legend = bottomleft)
    p4 = plot([0 0 0], showaxis = false, grid = false, legend = false)

    plot3 = plot(p1, p2, p3, p4, layout = (1,4), titlefontsize = 14, guidefontsize = 10, guide_position = :left, margin= 20Plots.mm, foreground_color_text = "white")
    plot!(plot3,size=(1500,2500))
    plot4 = plot(plot1, plot2, layout = (1, 3), titlefontsize = 14, guidefontsize = 10, guide_position = :left, margin= 20Plots.mm, markersize = 5.5, markercolor="blue", markerstrokewidth=0.2)
    plot!(plot4,size=(3750,2500))
    plot5 = plot(plot3,plot4, layout = (2, 1), titlefontsize = 14, guidefontsize = 10, guide_position = :left, margin= 20Plots.mm, markersize = 5.5, markercolor="blue", markerstrokewidth=0.2)
    plot!(plot5,size=(3000,3000))
    display(plot5)
