...
elseif leg_test == true
    p3 = plot([0 0 0], showaxis = false, grid = false, legend = false)
    p2 = plot([0 0 0], showaxis = false, grid = false, legend = false)
    p4 = plot([0 0 0], color = [Color_alt_1 Color_alt_2 Color_jalihal], showaxis = false, grid = false, labels = [label_alt_1 label_alt_2 label_jalihal],labelsize=100, legendfontsize=50, legend = :bottomright)
    ...
    #display(plot4)
    #savefig(plot4, pwd()*"/Results/Plots_for_new_and_old_paramter_values/plot_and_histogram_for_p_sets_with_legend_test.png")
